...
async def get_user_language(user_id):
    ...
    return status['language'] if status else "EN"
...
async def set_user_language(user_id, lang):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE members SET language = $1 WHERE user_id = $2",
            lang, user_id
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not lang or lang == "EN" and not await get_member_status(user_id):
        keyboard = [
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'pay_paypal_lifetime':
        await query.edit_message_text(
    ...
    # Crypto
    if query.data.startswith('pay_crypto'):
    ...
    if query.data.startswith('pay_stripe_'):
        ...
        price_id = LETMEBOT_PRICE_MONTHLY if plan == 'monthly' else LETMEBOT_PRICE_LIFETIME
        ...
        session = stripe.checkout.Session.create(
            ...
            metadata={'user_id': user_id, 'bot_name': 'letmebot'}  # 중요!
        )
        await query.edit_message_text(
