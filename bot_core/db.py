...
async def log_action(user_id, action, amount=0):
    """사용자 행동 / 결제 기록"""
    conn = await asyncpg.connect(DATABASE_URL)
    await conn.execute('INSERT INTO daily_logs (user_id, action, amount) VALUES ($1, $2, $3)',
...
async def get_daily_stats():
    ...
    ''', today)
    await conn.close()
    return row if row else {'unique_users': 0, 'total_revenue': 0}
