import os

# Stripe
STRIPE_SECRET_KEY = os.getenv("STRIPE_SECRET_KEY")
STRIPE_WEBHOOK_SECRET = os.getenv("STRIPE_WEBHOOK_SECRET")

# Database
DATABASE_URL = os.getenv("DATABASE_URL")

# Telegram Bots Tokens
LETMEBOT_TOKEN = os.getenv("LETMEBOT_TOKEN")
MOREVIDS_TOKEN = os.getenv("MOREVIDS_TOKEN")
ONLYTRNS_TOKEN = os.getenv("ONLYTRNS_TOKEN")
TSWRLDBOT_TOKEN = os.getenv("TSWRLDBOT_TOKEN")
LUST4TRANS_TOKEN = os.getenv("LUST4TRANS_TOKEN")

# Stripe Price IDs
LETMEBOT_PRICE_MONTHLY = os.getenv("LETMEBOT_PRICE_MONTHLY")
LETMEBOT_PRICE_LIFETIME = os.getenv("LETMEBOT_PRICE_LIFETIME")
LETMEBOT_PRICE_WEEKLY = os.getenv("LETMEBOT_PRICE_WEEKLY")
MOREVIDS_PRICE_MONTHLY = os.getenv("MOREVIDS_PRICE_MONTHLY")
MOREVIDS_PRICE_LIFETIME = os.getenv("MOREVIDS_PRICE_LIFETIME")
MOREVIDS_PRICE_WEEKLY = os.getenv("MOREVIDS_PRICE_WEEKLY")
ONLYTRNS_PRICE_LIFETIME = os.getenv("ONLYTRNS_PRICE_LIFETIME")
TSWRLDBOT_PRICE_LIFETIME = os.getenv("TSWRLDBOT_PRICE_LIFETIME")
LUST4TRANS_PRICE_WEEKLY = os.getenv("LUST4TRANS_PRICE_WEEKLY")
LUST4TRANS_PRICE_MONTHLY = os.getenv("LUST4TRANS_PRICE_MONTHLY")
LUST4TRANS_PRICE_LIFETIME = os.getenv("LUST4TRANS_PRICE_LIFETIME")

# Portal Return URLs
LETMEBOT_PORTAL_RETURN_URL = os.getenv("LETMEBOT_PORTAL_RETURN_URL")
MOREVIDS_PORTAL_RETURN_URL = os.getenv("MOREVIDS_PORTAL_RETURN_URL")
ONLYTRNS_PORTAL_RETURN_URL = os.getenv("ONLYTRNS_PORTAL_RETURN_URL")
TSWRLDBOT_PORTAL_RETURN_URL = os.getenv("TSWRLDBOT_PORTAL_RETURN_URL")
LUST4TRANS_PORTAL_RETURN_URL = os.getenv("LUST4TRANS_PORTAL_RETURN_URL")

# Channel and Admin
CHANNEL_ID = int(os.getenv("CHANNEL_ID", "0"))  # int 변환 추가
ADMIN_USER_ID = int(os.getenv("ADMIN_USER_ID", "0"))  # int 변환 추가
LUST4TRANS_PROMOTER_ID = int(os.getenv("LUST4TRANS_PROMOTER_ID", "0"))  # int 변환 추가 (일관성 위해)
TSWRLDBOT_PROMOTER_ID = int(os.getenv("TSWRLDBOT_PROMOTER_ID", "0"))  # int 변환 추가 (일관성 위해)

# Crypto and PayPal
CRYPTO_ADDRESS = os.getenv("CRYPTO_ADDRESS")
CRYPTO_QR_URL = os.getenv("CRYPTO_QR_URL")

PAYPAL_LETME_MONTHLY = os.getenv("PAYPAL_LETME_MONTHLY")
PAYPAL_LETME_LIFETIME = os.getenv("PAYPAL_LETME_LIFETIME")
PAYPAL_LETME_WEEKLY = os.getenv("PAYPAL_LETME_WEEKLY")
PAYPAL_MOREVIDS_MONTHLY = os.getenv("PAYPAL_MOREVIDS_MONTHLY")
PAYPAL_MOREVIDS_LIFETIME = os.getenv("PAYPAL_MOREVIDS_LIFETIME")
PAYPAL_MOREVIDS_WEEKLY = os.getenv("PAYPAL_MOREVIDS_WEEKLY")
PAYPAL_ONLYTRNS = os.getenv("PAYPAL_ONLYTRNS")
PAYPAL_TSWRLD = os.getenv("PAYPAL_TSWRLD")
PAYPAL_LUST4TRANS_WEEKLY = os.getenv("PAYPAL_LUST4TRANS_WEEKLY")
PAYPAL_LUST4TRANS_MONTHLY = os.getenv("PAYPAL_LUST4TRANS_MONTHLY")
PAYPAL_LUST4TRANS_LIFETIME = os.getenv("PAYPAL_LUST4TRANS_LIFETIME")

# Welcome Videos
WELCOME_VIDEO_MOREVIDS = os.getenv("WELCOME_VIDEO_MOREVIDS")
WELCOME_VIDEO_ONLYTRNS = os.getenv("WELCOME_VIDEO_ONLYTRNS")
WELCOME_VIDEO_TSWRLD = os.getenv("WELCOME_VIDEO_TSWRLD")
WELCOME_VIDEO_LUST4TRANS = os.getenv("WELCOME_VIDEO_LUST4TRANS")

# Render External URL (웹훅용 - 필수!)
RENDER_EXTERNAL_URL = os.getenv("RENDER_EXTERNAL_URL")

# Plan Prices (View Plans에서 표시)
PLAN_PRICES = {
    'letmebot': {'weekly': '$10', 'monthly': '$20', 'lifetime': '$50'},
    'morevids': {'weekly': '$10', 'monthly': '$20', 'lifetime': '$50'},
    'onlytrns': {'lifetime': '$25'},
    'tswrld': {'lifetime': '$21'},
    'lust4trans': {'weekly': '$11', 'monthly': '$21', 'lifetime': '$52'},
}
